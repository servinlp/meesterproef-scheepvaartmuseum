function setUpMap(){
	...
	let pathsOfActiveYear = document.querySelectorAll(`#year${currentActiveYear.dataset.year} > g > path`)
	...
	function initializeMap(){
		...
		allPaths.forEach( (path, i) => {
			...
			if( path === path.parentNode.querySelectorAll('path')[lastPathIndex] && !path.classList.contains('path-clone') ) {
				console.log('path is currently the last child of the parent');
	...
	function addMapFunctionality() {
		console.log('add Map Func FIRED!');
		...
		pathsOfActiveYear.forEach( (path, i) => {
			// Show current path
		...
		const allRouteAnimations = allBezierRoutes.forEach( (arr) => {
